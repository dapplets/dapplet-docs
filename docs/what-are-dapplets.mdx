...
The key factor is that you don't have to be a _YouTube_ or _Twitter_ developer to add functionality that you thought was missing for so long or is relevant right now. Or maybe someone has already implemented it? Then it’s easy enough to find and enable the corresponding **dapplet**!

The name **dapplet** consists of two parts: **_dapp_** and **_applet_**.
...
An **_applet_** is an application that exists in the context of another, larger web application and performs a specific task. **Dapplets** do not exist separately from the contexts for which they are written. They can be said to mutate or augment existing popular resources.
